use crate::{
    ...
    types::{constants::WORKFLOW_EXPAND, mixed_fs::MixedFS},
};
use anyhow::Result;
...
pub fn get_expanded_mixed_fs(mut mixed_fs: MixedFS, workflows_path: PathBuf) -> Result<MixedFS> {
    ...
    let expand_workflow_path = workflows_path.join(WORKFLOW_EXPAND);
    log!("Debug:Parse expand workflow at {expand_workflow_path:?}");
    if expand_workflow_path.exists() {
        log!("Debug:Expanding mixed_fs with {WORKFLOW_EXPAND} at {expand_workflow_path:?}");
