...
use std::ffi::CString;
use winapi::um::winuser::{FindWindowA, GetWindowTextA, GetWindowTextLengthA, IsWindowVisible, EnumWindows};
...
fn enum_windows_titles() {
    unsafe {
        ...
        EnumWindows(Some(enum_windows_titles_callback), &mut window_titles as *mut _ as _);
...
unsafe extern "system" fn enum_windows_titles_callback(window_handle: winapi::shared::windef::HWND, lParam: winapi::shared::minwindef::LPARAM) -> winapi::shared::minwindef::BOOL {
    ...
    if title_length > 0 {
        ...
        let title = CString::from_vec_unchecked(title_buffer).into_string().unwrap();
